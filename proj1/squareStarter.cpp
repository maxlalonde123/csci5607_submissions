...
unsigned char* loadImage(int& img_w, int& img_h){
   ...
   for (int i = 0; i < img_h; i++){
      ...
      for (int j = 0; j < img_w; j++){
         ...
         img_data[i*img_w*4 + j*4 + 1] = min(255.0f, B * tex_brightness);  //Green
         img_data[i*img_w*4 + j*4 + 2] = min(255.0f, G * tex_brightness);  //Blue
         img_data[i*img_w*4 + j*4 + 3] = min(255.0f, R * tex_brightness);  //Alpha
      }
   }

   //This is becauce glTexImage() assumes a bottome left origin. Reverse() is pulled from the std::algorithm library
   reverse(img_data, img_data + 4*img_w*img_h);
...
void mouseClicked(float m_x, float m_y){   
   ...
   Point2D corner1 = Point2D(p1.x, p1.y); //Bottom left
   Point2D corner2 = Point2D(p2.x, p2.y); //Bottom right
   Point2D corner3 = Point2D(p3.x, p3.y); //Top right
   Point2D corner4 = Point2D(p4.x, p4.y); //Top left

   float click_to_scale_or_rotate_distance = dist(corner4, corner3) * 0.04;

   //Check to see if click is inside the square
   if (!(segmentSegmentIntersect(clicked_pos, clicked_mouse, p4, p3)) &&
       !(segmentSegmentIntersect(clicked_pos, clicked_mouse, p3, p2)) &&
       !(segmentSegmentIntersect(clicked_pos, clicked_mouse, p2, p1)) &&
       !(segmentSegmentIntersect(clicked_pos, clicked_mouse, p1, p4)))
   {
      //Check if the click is close enought to the corner
      if (dist(clicked_mouse, p1) < click_to_scale_or_rotate_distance ||
          dist(clicked_mouse, p2) < click_to_scale_or_rotate_distance ||
          dist(clicked_mouse, p3) < click_to_scale_or_rotate_distance ||
          dist(clicked_mouse, p4) < click_to_scale_or_rotate_distance)
      {
         do_translate = false;
         do_scale = true;
         do_rotate = false;
      ...
      //If the click was not near the corner, check if its near an edge
      else if(dist(clicked_mouse, join(p4, p3)) < click_to_scale_or_rotate_distance ||
              dist(clicked_mouse, join(p3, p2)) < click_to_scale_or_rotate_distance ||
              dist(clicked_mouse, join(p2, p1)) < click_to_scale_or_rotate_distance ||
              dist(clicked_mouse, join(p1, p4)) < click_to_scale_or_rotate_distance)
      {
         do_translate = false;
         do_scale = false;
         do_rotate = true;
...
void mouseDragged(float m_x, float m_y){
   ...
   if (do_translate){
      ...
      //look at clicked_pos and get distance from clicked position to rect_pos and subtract that from distance between current position and origin.
      //That difference will replace dist(clicked_mouse, cur_mouse)

      Line2D l1 = join(rect_pos, clicked_mouse).normalized();
      Line2D l2 = join(clicked_mouse, cur_mouse).normalized();

      if (dot(l1, l2) < 0)
      {
         if (clicked_size - dist(clicked_mouse, cur_mouse) > 0.02)
         {
            rect_scale = clicked_size - dist(clicked_mouse, cur_mouse);
         }         
      }
      else if (dot(l1, l2) > 0)
      {
      ...
      float offsetX = rect_pos.x;
      float offsetY = rect_pos.y;
      
      rect_angle = clicked_angle + (-1 * atan2(cur_mouse.y - offsetY, cur_mouse.x - offsetX)) - (-1 * atan2(clicked_mouse.y - offsetY, clicked_mouse.x - offsetX)); //This is wrong: the angle should change based on the mouse movements
   }
